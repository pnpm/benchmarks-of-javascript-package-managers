...
module.exports = (resultArrays, pms, tests) => {
  ...
  graphLines.forEach((graphLine, index) => {
    ...
    const y2 = y1 + graph.h
    const line = `  <line x1="${graphLine}" y1="${y1}" x2="${graphLine}" y2="${y2}" stroke-width="${strokeWidth}" stroke="${graphColor}"></line>` + '\n'
    ...
    svgStr += `  <text x="${x}" y="${y}" fill="${graphColor}" font-size="${fontSize}" font-family="${fontFamily}" text-anchor="${anchor}">${number}</text>` + '\n'

    // add numbers below the graph lines
    const baseline = 'hanging'
    y = y2 + 1
    svgStr += `  <text x="${x}" y="${y}" fill="${graphColor}" font-size="${fontSize}" font-family="${fontFamily}" text-anchor="${anchor}" alignment-baseline="${baseline}">${number}</text>` + '\n'
  })
  ...
  ;(() => {
    ...
    const y = graph.y - 15
    svgStr += `  <text x="${x}" y="${y}" font-size="${fontSize}" font-family="${fontFamily}" font-style="${fontStyle}" text-anchor="${anchor}" fill="${graphColor}">${text}</text>` + '\n'
  })()
